...
export default class AchievementsProvider extends BaseProvider {
    ...
    async getAchievements(xuid:string, continuationToken = undefined, maxItems = undefined, skipItems = undefined): Promise<AchievementsResponse> {
        return (await this.get(this.applyPagination('/users/xuid('+xuid+')/history/titles', maxItems, skipItems, continuationToken)))
    }
    
    async getTitleId(xuid:string, titleId:string, continuationToken = undefined, maxItems = undefined, skipItems = undefined): Promise<AchievementsTitleResponse> {
    ...
    async getItemDetail(xuid:string, serviceConfigId:string, achievementId:string, continuationToken = undefined, maxItems = undefined, skipItems = undefined): Promise<AchievementsTitleResponse> {
